...
EX1 <- BBS[, 
           list(vernacularName, 
                individualCount)]
```
這邊給個小練習，試看看以下兩個取欄位的寫法結果跟上面有沒有不同
`EX1.1 <- BBS[, c(vernacularName, individualCount)]`
...
```
###DT[DT2, on = "a"] 
####利用相同的欄位**a**來合併兩個data.table，**DT**與**DT2**
EX 
將鳥類名錄中的遷留屬性(Bird.list$Taiwan)對應到BBS資料(BBS)當中
```{r warning = FALSE}
